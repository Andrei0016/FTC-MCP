...
def start_ftc_project(target_dir: str, team_number: str = "") -> str:
    ...
    return (
        ...
        "5. For the first subsystem: ask the user what's needed to understand it, then "
        "ask whether it needs a plan; if yes, confirm your understanding and approach "
        "with them, draft a plan, run BOTH planning agents, reconcile, present the "
        "reconciled plan to the user for approval, then use the new_subsystem tool.\n"
...
def ftc_change_workflow(request: str) -> str:
    ...
    return (
        ...
        "3. If non-trivial: ask the user whether it needs a plan before drafting one. If "
        "yes, confirm your understanding and intended approach with the user, draft a "
        "plan, run plan-pro-advocate AND plan-critic, reconcile, then present the "
        "reconciled plan to the user and get their go-ahead before coding.\n"
